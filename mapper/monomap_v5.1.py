...
# helper function to map recurrent cycles first, then rest in topological order
# cyclic nodes are detected from SCCs in the original directed DFG
def recurrent_first_order(dfg, topo_order):
    ...
    for scc in sccs:
        if len(scc) > 1:
            ...
            n = next(iter(scc))
            if dfg.has_edge(n, n):   # self-loop case
                recurrent_nodes.add(n)

    cycle_first = [n for n in topo_order if n in recurrent_nodes]
    rest = [n for n in topo_order if n not in recurrent_nodes]
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    # compute a fixed topological order from an acyclic DFG
    # Loop-carried dependencies are ignored for this ordering,
    # just like in ASAP/ALAP scheduling
    ...
    topo_order = list(nx.topological_sort(dfg_for_order))
    map_order = recurrent_first_order(dfg, topo_order)
