...
#               COST. Given these information we draw a random node following a poisson distribution from the
#               worst nodes, the node that we get is the one that will be moved. We maintain the current solution
...
def cost_space_solution(node_pe: dict[int, int], pe_nodes: dict[int, list[int]], dfg: DiGraph, arch: Graph, size_x: int, size_y: int) -> tuple[int, list[int]]:
    """
...
def simulatedAnnealingSearch(schedule: dict[str, list[int]], dfg: DiGraph, arch: Graph, II: int, size_x: int, size_y: int) -> tuple[dict[int, int],  dict[int, list[int]], float]:
    ...
    sol_cost, sol_worst_nodes = cost_space_solution(node_pe, pe_nodes, dfg, arch, size_x, size_y)
    ...
    while FREEZING_TEMPERATURE < TEMPERATURE and sol_cost != 0:
        ...
        c, curr_sol_worst_nodes = cost_space_solution(curr_node_pe, curr_pe_nodes, dfg, arch, size_x, size_y)
    ...
    # Plot search data
    saveFigTemperature(temperatures, costs, costs_sma_201, costs_sma_5)
