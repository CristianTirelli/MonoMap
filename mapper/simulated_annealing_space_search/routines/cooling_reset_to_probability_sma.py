...
# Same as cooling_reset_sma.py
# With the addition of a reset towards a designated probability

@dataclass
class FixedCoolingResetToProbabilitySma(SimulatedAnnealingSpaceSearch):
    TEMPERATURE_STRATEGY_ID: str = field(default=None, init=False)

    RESET_PROBABILITY: float = field(default=0.3, init=False)
    ...
    def __post_init__(self):
        ...
        first = np.linspace(100, 50, 75, endpoint=False)
        second = np.linspace(50, 25, 125, endpoint=False)
        third = np.linspace(25, 10, 250, endpoint=False)
        fourth = np.linspace(10, 0.001, 500, endpoint=False)
        steps = np.concatenate([first, second, third, fourth])
        ...
        for i, stp in enumerate(steps):
            ...
            coeff_items = (self.START_TEMPERATURE - stp) / 100
            additional_items = int(
                ((coeff_items ** 2 if coeff_items > 1 else coeff_items) * (self.ITEMS_PER_TEMPERATRE))
            )
            self.PROBABILITIES_TABLE.append((stp, self.ITEMS_PER_TEMPERATRE + additional_items if additional_items < MAX_CAP else MAX_CAP))

        self.TEMPERATURE_STRATEGY_ID = f"FIXED-P-RESET-TO-{str(self.RESET_PROBABILITY).replace(".", "-")}_"
        ...
        while self.sol_cost != 0 and self.iterations < self.MAX_ITERATIONS:
            ...
            if self.t_index + 1 == self.PROBABILITIES_STEPS:
                ...
                if plateau_times >= self.MAX_PLATEAU_TIMES:
                    ...
                    seeked_p = self.RESET_PROBABILITY * 100
                    best_fit_delta = 101
                    best_fit_idx = 0
...
class CoolingResetToProbabilityDynamicLearnedSma(SimulatedAnnealingSpaceSearch):
    ...
    def __post_init__(self):
        ...
        self.TEMPERATURE_STRATEGY_ID = f"COOLING-RESET-TO-{str(self.RESET_PROBABILITY).replace(".", "-")}-DYNAMIC-LEARNED_"
...
@dataclass
class CoolingResetToProbabilityDynamicSma(SimulatedAnnealingSpaceSearch):
    TEMPERATURE_STRATEGY_ID: str = field(default=None, init=False)

    # do 0.05 at this seed
    RESET_PROBABILITY: float = field(default=0.8, init=False)
    ...
    def __post_init__(self):
        ...
        self.TEMPERATURE_STRATEGY_ID = f"COOLING-RESET-TO-{str(self.RESET_PROBABILITY).replace(".", "-")}-DYNAMIC-BEST-COST_"
