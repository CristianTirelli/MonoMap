...
# -----------------------------------------------------------------------------
def backtracking(dfg_undir: nx.Graph, arch: nx.Graph):
    ...
    # Set of currently unassigned DFG nodes
    # (used instead of repeatedly checking "v not in assign") 
    unassigned = set(dfg_undir.nodes)

    # keeping domains incrementally in dictionary
    domains = {v: set(base_domains[v]) for v in dfg_undir.nodes}

    # Reverse mapping: architecture node -> set of DFG nodes that can map to it
    arch_to_vars = defaultdict(set)
    for v in dfg_undir.nodes:
        for a in base_domains[v]:
            arch_to_vars[a].add(v)
    ...
    def pick_next():
        ...
        for v in unassigned:
            candidates.append((len(domains[v]), -dfg_undir.degree(v), v))
    ...
    # undo all changes recorded on the trail
    # the trail stores reversible updates made during assignment and propagation,
    # so that failed search branches can restore the previous state exactly.
    def undo(trail):
        for typ, *rest in reversed(trail):
            if typ == "domain_remove":
                u, val = rest
                domains[u].add(val)
            elif typ == "assign":
                v = rest[0]
    ...
    # -------------------------------------------------------------------------
    # assign v -> a and propagate the consequences of that decision
    #
    # This performs incremental constraint propagation:
    #  1. Record the assignment
    #  2. Restrict domain(v) to {a}
    #  3. Remove a from all other unassigned variables (injectivity)
    #  4. Enforce adjacency constraints on unassigned neighbors of v
    #
    # the modifications are recorded in a trail so they can be undone later
    # during backtracking
    # -------------------------------------------------------------------------
    def assign_and_propagate(v, a):
        trail = []
        ...
        # domain(v) becomes {a}
        for x in list(domains[v]):
            if x != a:
                domains[v].remove(x)
                trail.append(("domain_remove", v, x))

        # architecture node a cannot remain in the domain of any other
        # still-unassigned variable (injectivity)
        for u in arch_to_vars[a]:
            if u in unassigned and a in domains[u]:
                domains[u].remove(a)
                trail.append(("domain_remove", u, a))
                if not domains[u]:
                    return False, trail

        # every unassigned neighbor u of v must be adjacent to a (adjacency)
        allowed = arch_nbrs[a]
        for u in dfg_undir.neighbors(v):
            if u in unassigned:
                for x in list(domains[u]):
                    if x not in allowed:
                        domains[u].remove(x)
                        trail.append(("domain_remove", u, x))
                if not domains[u]:
                    return False, trail
    ...
    def dfs():
        ...
        values = sorted(
            ...
            key=lambda a: sum(
                ...
                if u not in assign and a in domains[u]
            )
        )

        for a in values:
            # try to assign v -> a and propagate the resulting constraints
            ok, trail = assign_and_propagate(v, a)

            debug_print("Assign", v, "->", a)

            # Continue search only if:
            # propagation succeeded,
            # forward checking detects no empty domains,
            # and the recursive search succeeds
            if ok and forward_check() and dfs():
                return True

            # otherwise, this branch failed so undo all assignment/propagation 
            # changes and try the next value
            debug_print("Backtracking on", v)
            undo(trail)

        # no candidate value led to a complete valid mapping
        return False

    return assign if dfs() else None
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    m_dfg_to_arch = backtracking(dfg, arch)
    end = time.time()
...
def main():
    ...
    debug_print("#nodes: ",len(dfg.nodes))
    debug_print("#edges: ",len(dfg.edges))
    debug_print("#maxdegree: ", getMaxOutDegree(dfg))
    if int(args.i) != -1:
        II = int(args.i)
