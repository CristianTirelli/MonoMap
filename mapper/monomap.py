...
def generatePKE(schedule_it, II):
    ...
    for t in schedule_it:
        for it in schedule_it[t]:
            idx = t + it * II
        ...
        if s.check() == sat:
            ...
            for t in m.decls():
                ...
                if is_true(m[t]):
                    ...
                    if cycle not in schedule_it:
                        schedule_it[cycle] = {}
                    
                    if iteration not in schedule_it[cycle]:
                        schedule_it[cycle][iteration] = []

                    schedule_result[cycle].append(nodeid)
                    schedule_it[cycle][iteration].append(nodeid)
        else:
            print("UNSAT")   
        end = time.time()
