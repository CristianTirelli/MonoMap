...
def backtracking(dfg_undir, arch, size_x, size_y):
    ...
    def pick_next():
        ...
        # Build candidate list (same as your original code)
        for v in dfg_undir.nodes:
            if v not in assign:
                dom = current_domain(v)
                degree = dfg_undir.degree(v)
                candidates.append((len(dom), -degree, v, dom))
        ...
        if len(after_mrv) == 1:
            ...
            # Tiebreak 1: degree (highest degree → smallest negative degree)
            min_neg_deg = None
            ...
            if len(after_degree) == 1:
                ...
                # Tie-break2 (fallback): node id fallback
    ...
    def dfs():
        ...
        #   "max" -> prefer more loaded PEs first
        #
        # secondary criteria:
        # keeping the old v1 least-constraining value heuristic as a tie-breaker
        # -------------------------------------------------------------
        pressure_mode = "max"   # "min" for the opposite
        ...
        def pressure_score(a):
            pe = a % (size_x * size_y)
            return pe_pressure[pe]

        if pressure_mode == "min":
            values = sorted(
                dom,
                key=lambda a: (
                    pressure_score(a),   # less loaded PE first
                    lcv_score(a),        # then least-constraining value
                    a                    
                )
            )
        else:
            values = sorted(
                dom,
                key=lambda a: (
                    -pressure_score(a),  # more loaded PE first
                    lcv_score(a),        # then least-constraining value
                    a
                )
            )
