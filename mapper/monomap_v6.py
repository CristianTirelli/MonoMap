...
# -----------------------------------------------------------------------------
# BACKTRACKING + time-step child-guided node ordering (J)
...
#  - dynamic domains
#  - fixed time-step + child-guided node ordering
#  - forward checking
...
def backtracking(dfg_undir, arch, size_x, size_y, map_order):
    ...
    # VARIABLE SELECTION: MRV + time-child guidance
    #
    # Strategy:
    # - select the unassigned node with the smallest current domain (MRV)
    # - break ties using the precomputed time-child ordering
    #
    # Additional tie-breaks:
    # - most descendants first
    # - higher graph degree
    # - topological rank
    def pick_next():
        for v in map_order:
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    # computing a fixed topological order from an acyclic view of the DFG
    # loop-carried dependencies are ignored for this ordering,
    ...
    #   2. then map all their immediate children before proceeding with t+=1
    def time_child_order(graph, schedule, topo_rank):
        # final mapping order
        order = [] 
        ...
        for t in sorted(schedule):
            ...
            # get nodes scheduled at time t
            # sort them using topological rank
            parents = sorted(
                schedule[t],
                key=lambda v: topo_rank.get(v, float('inf'))
            )
            ...
            # step 2: collect immediate children of these nodes
            children = []
            for parent in parents:
                for child in graph.successors(parent):  # direct neighbors only
                    children.append(child)

            # step 3: add children (sorted)
            for child in sorted(
                children,
                key=lambda v: topo_rank.get(v, float('inf'))
            ):
    ...
    map_order = time_child_order(dfg_for_order, schedule, topo_rank)
