...
# -----------------------------------------------------------------------------
# BACKTRACKING + MRV with time-child-rank tie-breaking (J)
...
#  - dynamic domains
#  - MRV node ordering
#  - time-child-rank tie-breaking
#  - descendants-count tie-breaking
#  - degree-based tie-breaking
#  - topological-rank tie-breaking
#  - forward checking
#  - least-constraining value ordering
# -----------------------------------------------------------------------------
def backtracking(dfg_undir, dfg_directed, arch, size_x, size_y):
    ...
    # number of descendants (reachable nodes) for each node
    descendants_count = {
        v: len(nx.descendants(dfg_directed, v))
        for v in dfg_directed.nodes
    }

    # build an acyclic DFG view for ordering by removing loop-carried back edges
    dfg_for_order = dfg_directed.copy()
    ...
    topo_rank = {}
    for i, v in enumerate(nx.topological_sort(dfg_for_order)):
        topo_rank[v] = i

    def build_time_child_rank():
        order = []
        seen = set()
        ...
        # build time -> nodes from DFG node attributes
        nodes_by_time = defaultdict(list)
        for v in dfg_undir.nodes:
            nodes_by_time[dfg_undir.nodes[v]["time"]].append(v)

        for t in sorted(nodes_by_time):
            parents = sorted(
                nodes_by_time[t],
                key=lambda v: topo_rank.get(v, float("inf"))
            )

            # add nodes scheduled at this time
            for parent in parents:
                add(parent)

            # add their immediate children from the acyclic directed graph
            children = []
            for parent in parents:
                if parent in dfg_for_order:
                    children.extend(dfg_for_order.successors(parent))
    ...
    time_child_rank = build_time_child_rank()
    ...
    #
    # Tie-break: most descendants first, 
    # then higher graph degree second (more connected nodes are likely harder)
    # -------------------------------------------------------------------------
    def pick_next():
        candidates = []
        ...
        for v in dfg_undir.nodes:
            if v not in assign:
                dom = current_domain(v)

                candidates.append((
                    len(dom),                                   
                    time_child_rank.get(v, float("inf")),    
                    -descendants_count.get(v, 0),                
                    -dfg_undir.degree(v),                          
                    topo_rank.get(v, float("inf")),                
                    v,
                    dom
                ))
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    m_dfg_to_arch, pe_pressure = backtracking(dfg, dfg_directed, arch, size_x, size_y)
    end = time.time()
