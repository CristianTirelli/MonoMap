...
def backtracking(dfg_undir, arch, size_x, size_y):
    ...
    pick_stats = {
        ...
        "main_decisive": 0,
        "tiebreak1_needed": 0,
        "tiebreak1_decisive": 0,
        "tiebreak2_needed": 0,
        "tiebreak2_decisive": 0,
        "fallback_needed": 0,
        "fallback_decisive": 0,
    }
    ...
    # -------------------------------------------------------------------------
    # Identify the busiest time step among currently unassigned DFG nodes
    #
    # Algo strategy:
    # - count how many unassigned nodes remain at each time label
    # - return the time step with the largest count
    #
    # Idea:
    # - start search from the most populated time step, since it is likely
    #   to be more constrained
    # -------------------------------------------------------------------------
    def busiest_unassigned_time():
    ...
    # -------------------------------------------------------------------------
    # Pick the next DFG node to assign
    #
    # Algo:
    # 1. restrict attention to the busiest unassigned time step
    # 2. within that time step, apply MRV (Minimum Remaining Values):
    #    choose the node with the smallest current feasible domain
    # 3. tie-break by higher graph degree first
    #
    # this combines the "busiest time" heuristic with MRV
    # -------------------------------------------------------------------------
    def pick_next():
        pick_stats["pick_next_calls"] += 1

        target_time = busiest_unassigned_time()
        if target_time is None:
            return None, None

        candidates = []

        # only consider nodes at busiest time
        for v in dfg_undir.nodes:
            if v not in assign and dfg_undir.nodes[v]["time"] == target_time:
        ...
        # Main heuristic: busiest_time
        if len(candidates) == 1:
            pick_stats["main_decisive"] += 1

        else:
            pick_stats["tiebreak1_needed"] += 1

            # tie-break 2: MRV (smallest domain)
            min_dom = None
            ...
            if len(after_mrv) == 1:
                pick_stats["tiebreak1_decisive"] += 1

            else:
                pick_stats["tiebreak2_needed"] += 1

                # tie-break 3: highest-degree
                min_neg_deg = None

                for c in after_mrv:
                    neg_deg = c[1]

                    if min_neg_deg is None:
                        min_neg_deg = neg_deg
                    else:
                        if neg_deg < min_neg_deg:
                            min_neg_deg = neg_deg
                ...
                for c in after_mrv:
                    if c[1] == min_neg_deg:
                        after_degree.append(c)

                if len(after_degree) == 1:
                    pick_stats["tiebreak2_decisive"] += 1

                else:
                    # tie-break 4(fallback): node id fallback
    ...
    if dfs():
        return assign, pe_pressure, pick_stats
    else:
        return None, None, pick_stats
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    m_dfg_to_arch, pe_pressure, pick_stats = backtracking(dfg, arch, size_x, size_y)
    end = time.time()
    ...
    # stats of heuristics
    print("Pick-next stats:", pick_stats)
