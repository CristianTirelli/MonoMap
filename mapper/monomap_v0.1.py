...
# -----------------------------------------------------------------------------
# BACKTRACKING SKELETON (J)
# This is a simple baseline DFS backtracking mapper.
# It tries to assign each DFG node to an architecture node while enforcing:
...
# NOTES:
# - Domains are initialized only from time labels; there is no forward checking or domain pruning yet.
# - Variable ordering uses a simple heuristic where we select a node from the "busiest" time step 
# (i.e., the time label with the most unassigned nodes), instead of picking the first unassigned node.
# -----------------------------------------------------------------------------
def backtracking(dfg_undir, arch, size_x, size_y):

    """
    Try to find a valid mapping from DFG nodes to architecture nodes using
    ...
    # Build initial domains for each DFG node.
    # At this stage, the only filtering criterion is time compatibility 
    # (no additional pruning is performed yet)
    domains = {}
    for v in dfg_undir.nodes:
        t = dfg_undir.nodes[v]["time"]
        domains[v] = list(arch_by_time[t])  
    ...
    # -----------------------------------------------------------------------------
    # VARIABLE SELECTION HEURISTIC (J)
    # Instead of picking the first unassigned DFG node,
    # we now prioritize nodes based on their "time" label:
    #
    # Strategy:
    # - Find the time step with the most unassigned DFG nodes
    # - Select a node from that time step
    #
    # Thoughts behind this idea:
    # Time steps with more nodes are typically more constrained so maybe
    # assigning them earlier may reduce branching and improve pruning
    # -----------------------------------------------------------------------------


    # Identify the "busiest" time step among unassigned DFG nodes.
    def busiest_unassigned_time():
        time_counts = defaultdict(int)

        # count how many unassigned nodes exist per time step
        for v in dfg_undir.nodes:
            if v not in assign:
                t = dfg_undir.nodes[v]["time"]
                time_counts[t] += 1

        # if no unassigned nodes remain, complete
        if not time_counts:
            return None
        
        # return the time with the maximum count
        return max(time_counts, key=time_counts.get)

    # -------------------------------------------------------------------------
    # Pick the next unassigned DFG node.
    # Strategy:
    # - Determine the busiest time step (most unassigned nodes)
    # - Return the first unassigned node belonging to that time step
    # This replaces the "first unassigned node" approach
    # -------------------------------------------------------------------------
    def pick_next(assign):
        # determine which time step to prioritize
        target_time = busiest_unassigned_time()
        if target_time is None:
            return None

        # pick the first unassigned node at that time step
        for v in dfg_undir.nodes:
    ...
    # Check whether assigning DFG node v -> architecture node a is consistent
    # with the current partial assignment. 
    # check injective + adjacency to already mapped neighbors
    def consistent(v, a):
        # if an arch ndoe has been used, reject reuse of it
        if a in used:
            return False
        # enforce adjacency with already assigned DFG neighbors
        for u in dfg_undir.neighbors(v):
            if u in assign:
                if assign[u] not in arch_nbrs[a]:
                    return False
        return True
