...
def backtracking(dfg_undir, arch, size_x, size_y):
    ...
    # 2. within that time step, apply highest degree heuristic:
    #    choose the node with the max no. of edges
    #
    # this combines the "busiest time" heuristic with highest degree
    # -------------------------------------------------------------------------
    def pick_next():
        ...
        for v in dfg_undir.nodes:
            if v not in assign and dfg_undir.nodes[v]["time"] == target_time:
                ...
                deg = dfg_undir.degree(v)
                if deg > best_deg:
                    best_deg = deg
                    best_v = v
                    best_dom = dom

        return best_v, best_dom
    ...
    def dfs():
        ...
        # value ordering: upwards (PE from low to high)
        values = sorted(dom)

        # another alternative to be used for benchmarks:
        # downwards (PE from high to low)
        # values = sorted(dom, reverse=True)
