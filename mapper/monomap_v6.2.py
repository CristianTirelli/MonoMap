...
# -----------------------------------------------------------------------------
# BACKTRACKING + time-step child-threshold node ordering (J)
...
#  - dynamic domains
#  - fixed time-step + child-threshold node ordering
#  - maps all descendants early when a node has more than child_threshold children
#  - forward checking
...
def backtracking(dfg_undir, arch, size_x, size_y, map_order):
    ...
    def current_domain(v):
        ...
        for u in dfg_undir.neighbors(v):
            if u in assign:
                # v must be mapped to a neighbor of assign[u]
    ...
    #
    # This replaces MRV. The search order is now fixed by data dependence.
    def pick_next():
        for v in map_order:
            if v not in assign:
    ...
    def dfs():
        ...
        debug_print(
            f"Trying next DFG node: {v}, "
            f"time={dfg_undir.nodes[v].get('time')}, "
            f"depth={len(assign)}"
        )
...
def map(dfg, arch, II, topology_degree, size_y, size_x, child_threshold):
    ...
    # Compute a fixed topological order from an acyclic view of the DFG.
    # Loop-carried dependencies (type="back_dep") are ignored for this ordering,
    # exactly like in ASAP/ALAP scheduling.
    dfg_for_order = dfg.copy()
    for u, v in get_back_edges(dfg_for_order):
        dfg_for_order.remove_edge(u, v)

    if not nx.is_directed_acyclic_graph(dfg_for_order):
        raise ValueError(
            "DFG still contains a cycle after removing back_dep edges; "
            "cannot compute topological order for mapping"
        )

    topo_rank = {}
    i = 0
    for v in nx.topological_sort(dfg_for_order):
        topo_rank[v] = i
        i += 1

    # build a mapping order s.t:
    # - For each time step t:
    #   1. map all nodes scheduled at time t
    #   2. For each of those nodes:
    #     - If it has more than child_threshold immediate children,
    #       map all descendants immediately, independently from time.
    #     - Otherwise, map only its immediate children, like v6 before proceeding with t+=1
    def time_child_order(graph, schedule, topo_rank, child_threshold):
        # final mapping order
        order = [] 

        # keeping track of already added ndoes       
        seen = set()
        ...
        def sorted_by_topo(nodes):
            ranked_nodes = []

            for node in nodes:
                if node in topo_rank:
                    rank = topo_rank[node]
                else:
                    rank = float('inf')
        ...
        # iterate over time steps in increasing order (t = 0, 1, 2, ...)
        for t in sorted(schedule):

            # nodes scheduled at current time step
            parents = sorted_by_topo(schedule[t])

            # step 1: add all nodes at current time step
            for parent in parents:
                add(parent)

            # step 2: apply the child_threshold policy
            for parent in parents:
                children = list(graph.successors(parent))

                if len(children) > child_threshold:
                    # map all descendants now, ignoring scheduled time
                    # for ordering purposes only
                    descendants = nx.descendants(graph, parent)

                    for node in sorted_by_topo(descendants):
                        add(node)

                else:
                    # only immediate children
                    for child in sorted_by_topo(children):
                        add(child)

        # make sure that any node not yet included is added at the end
        for node in sorted_by_topo(graph.nodes):
            add(node)

        return order
    
    map_order = time_child_order(dfg_for_order, schedule, topo_rank, child_threshold)
    print("Child threshold:", child_threshold)
    print("Mapping order:", map_order)
    ...
    m_dfg_to_arch, pe_pressure = backtracking(dfg, arch, size_x, size_y, map_order)
    end = time.time()
...
def main():
    ...
    parser.add_argument('-ct', type=int, help='If a node has more than this many children, map all descendants immediately', default=3)
    ...
    map(dfg, arch, II, topology_degree, CGRA_Y, CGRA_X, child_threshold)
