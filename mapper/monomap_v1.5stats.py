    ...
    def pick_next():
        ...
        for v in dfg_undir.nodes:
            if v not in assign:
                ...
                candidates.append((len(dom), -descendants_count[v], -dfg_undir.degree(v), v, dom))

        if not candidates:
            return None, None
        
        # Main heuristic: MRV (smallest domain size)
        min_dom = None
        ...
        if len(after_mrv) == 1:
            ...
            # Tiebreak 1: most descendants
            min_neg_desc = None

            for c in after_mrv:
                val = c[1]

                if min_neg_desc is None:
                    min_neg_desc = val
                else:
                    if val < min_neg_desc:
                        min_neg_desc = val
            ...
            if len(after_desc) == 1:
                ...
                # Tiebreak 2: degree (highest degree → smallest negative degree)
                min_neg_deg = None
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    m_dfg_to_arch, pe_pressure, pick_stats = backtracking(dfg, dfg_directed, arch, size_x, size_y)
    end = time.time()
