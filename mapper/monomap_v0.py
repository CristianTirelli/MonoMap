...
# - Variable ordering is currently naive: we simply pick the first unassigned DFG node.
# -----------------------------------------------------------------------------
def backtracking(dfg_undir, arch, size_x, size_y):
    ...
    # Current strategy:
    #   - simply return the first unassigned node encountered
    # For now this is simple and will later be replaced with a better heuristic.
    # -------------------------------------------------------------------------
    ...
    # Algo idea:
    #   1. Pick an unassigned DFG node
    #   2. Try each candidate architecture node in its domain
    #   3. If consistent, extend the partial assignment
    #   4. Recurse
    #   5. If recursion fails, undo the assignment and continue
    ...
    def dfs():
        ...
        # if no unassigned node remains, the mapping is complete
        if v is None:
            return True
        
        # try each candidate architecture node for v
        for a in domains[v]:
            if consistent(v, a):
                assign[v] = a
                used.add(a)
                ...
                # backtrack: undo the tentative assignment
        ...
        # no valid candidate worked for this node
        return False
