...
# helper function to compute the most critical path
def get_critical_path(dfg):
    g = dfg.copy()

    # remove the loops
    for u, v in get_back_edges(g):
        g.remove_edge(u,v)

    if not nx.is_directed_acyclic_graph(g):
        raise ValueError("DFG must be a DAG")
    ...
    def pick_next():
        ...
        for v in dfg_undir.nodes:
            if v not in assign:
                ...
                # nodes on the critical path are picked first
                if critical_rank is not None:
                    cp_rank = critical_rank.get(v, float("inf"))
                else:
                    cp_rank = float("inf")
                
                candidates.append((cp_rank, len(dom), -dfg_undir.degree(v), v, dom))
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    critical_path, critical_rank = get_critical_path(dfg)
    ...
    m_dfg_to_arch, pe_pressure = backtracking(dfg, arch, size_x, size_y, critical_rank)
    end = time.time()
