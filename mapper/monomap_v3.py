...
# -----------------------------------------------------------------------------
# BACKTRACKING + guided child/parent traversal + parent-PE priority
# This version extends the previous versions of the mapper with stronger
# constraint propagation, directed DFG traversal, and improved value ordering.
...
# - Local adjacency consistency: for every DFG edge (u, v), if one endpoint has already been mapped,
# the other endpoint must map to an adjacent architecture node

# Improvements over previous version:
#  - path-guided node ordering:
#       pick one node, go to a child, then go up to parents,
#       then continue down the path again
#  - pick a new node to start from when the current traversal path is exhausted
#  - value ordering with priority for PEs where mapped parent(s) are placed
#  - least-constraining value ordering as secondary criterion (from v1)
# -----------------------------------------------------------------------------
def backtracking(dfg, arch, size_x, size_y):
    ...
    #
    # Since the DFG is now kept directed, we explicitly store:
    # - predecessors = parents
    # - successors   = children
    # - nbrs         = union of both, used for adjacency feasibility checks
    # -------------------------------------------------------------------------
    preds = {v: list(dfg.predecessors(v)) for v in dfg.nodes}
    succs = {v: list(dfg.successors(v)) for v in dfg.nodes}
    nbrs  = {v: set(preds[v]) | set(succs[v]) for v in dfg.nodes}

    # return the PE index of a time-expanded architecture node
    def pe_of(a):
        return a % (size_x * size_y)
    ...
    def current_domain(v):
        dom = base_domains[v] - used

        # adjacency constraints from already assigned parents/children
        for u in nbrs[v]:
            if u in assign:
                dom &= arch_nbrs[assign[u]]

        return dom

    # Collect the PEs of already mapped parents of v,
    # this is used to prioritize candidate PEs that match where a parent is already mapped
    def mapped_parent_pes(v):
        parent_pes = []
        for p in preds[v]:
            if p in assign:
                parent_pes.append(pe_of(assign[p]))
        return parent_pes

    # -------------------------------------------------------------------------
    # SEED SELECTION

    # When the current traversal path cannot be extended anymore, start from a
    # new unassigned node.

    # Idea strategy:
    # - earliest time first
    # - then higher out-degree (nodes with more children)
    # - then higher total degree (most connected)
    # -------------------------------------------------------------------------
    def pick_seed():
        # start from earliest time first, then highest out-degree, then total degree
        candidates = [v for v in dfg.nodes if v not in assign]
        if not candidates:
            return None, None
        ...
        v = candidates[0]
        return v, current_domain(v)

    # -------------------------------------------------------------------------
    # PATH-GUIDED VARIABLE SELECTION
    
    # Strategy:
    # - extend the mapping from the current path instead of picking globally by MRV (v1)
    # - first try to map a child of the current node
    # - then go up to an unmapped parent and try to map it
    # - if neither is available, walk back along the path and continue from earlier nodes
    
    # Tie-break inside each candidate group:
    # - smaller current domain first (more constrained)
    # - higher degree first (more connected)
    # -------------------------------------------------------------------------
    def pick_from_path(path):
        if not path:
            return None, None

        last = path[-1]

        # 1) go down first: pick an unmapped child of current node
        child_candidates = [u for u in succs[last] if u not in assign]
        if child_candidates:
            child_candidates.sort(key=lambda u: (len(current_domain(u)), -dfg.degree(u), u))
            v = child_candidates[0]
            return v, current_domain(v)

        # 2) then go up: pick an unmapped parent of current node
        parent_candidates = [u for u in preds[last] if u not in assign]
        if parent_candidates:
            parent_candidates.sort(key=lambda u: (len(current_domain(u)), -dfg.degree(u), u))
            v = parent_candidates[0]
            return v, current_domain(v)

        # 3) walk back through the mapped path and find the next child
        for x in reversed(path[:-1]):
            child_candidates = [u for u in succs[x] if u not in assign]
            if child_candidates:
                child_candidates.sort(key=lambda u: (len(current_domain(u)), -dfg.degree(u), u))
                v = child_candidates[0]
                return v, current_domain(v)

            parent_candidates = [u for u in preds[x] if u not in assign]
            if parent_candidates:
                parent_candidates.sort(key=lambda u: (len(current_domain(u)), -dfg.degree(u), u))
                v = parent_candidates[0]
                return v, current_domain(v)
    ...
    # try to extend the current path; if not possible, choose a new node to continue mapping
    def pick_next(path):
        # try guided traversal first
        v, dom = pick_from_path(path)
        if v is not None:
            return v, dom
    ...
    def forward_check():
        for v in dfg.nodes:
            if v not in assign and len(current_domain(v)) == 0:
                return False
        return True

    def dfs(path):
        v, dom = pick_next(path)
        if v is None:
            return True

        # fail early if current node has no valid candidates
        if len(dom) == 0:
            return False
        ...
        values = sorted(
            dom,
            key=lambda a: (
                -sum(1 for pp in parent_pes if pe_of(a) == pp),  # prefer same PE as mapped parent(s)
                sum(
                    1 for u in nbrs[v]
                    if u not in assign and a in current_domain(u)
                )
            )
        )
        ...
        for a in values:
            ...
            if forward_check() and dfs(path + [v]):
                return True
...
def map(dfg, arch, II, topology_degree, size_y, size_x):
    ...
    # removing this line so that I can distinguish parents from children
